...
public class ServidorChat extends Thread {
    ...
    private static final ArrayList<PreguntaMessage> preguntas = new ArrayList<PreguntaMessage>();
    ...
    public void enviarPregunta( PreguntaMessage preguntaMessage ) {
        synchronized( clientes ) {
            for (AtenderClienteServidor atenderClienteServidor : clientes) {
                atenderClienteServidor.sendPregunta(preguntaMessage);
            }
        }
        synchronized( preguntas ) {
            preguntas.add(preguntaMessage);
        }
    }
    
    public void borrarPregunta( PreguntaMessage preguntaMessage ) {
        synchronized(preguntas) {
            preguntas.remove(preguntaMessage);
        }
    }
    
    public void enviarRespuestaPregunta( int idPregunta, String respuesta ) {
        synchronized (preguntas) {
            for ( PreguntaMessage preguntaMessage:preguntas ) {
                if ( idPregunta == preguntaMessage.getIdPregunta() ) {
                    preguntaMessage.getReceptorPregunta().enviarResultadoPregunta(respuesta);
                }
            }
        }
    }
